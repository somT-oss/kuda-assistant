...
app = Celery('kuda app', broker=os.getenv('REDIS_URL'), backend=os.getenv('REDIS_URL'))
...
@app.task
def convert_to_excel(transaction_content: Dict, transaction_type: str, start_date: str, end_date: str):
    ...
    try:
        ...
        logger.info(f"failed to write excel content to the file: {full_path}")
        return f"{e}"
...
def send_email(email: str, transaction_type: str, start_date: str, end_date: str) -> None:    
    ...
    msg.attach(MIMEText(f"This email contains a transaction history of {transaction_type} from {start_date} to {end_date}", 'plain'))
