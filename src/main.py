...
class CreditMetadata(TypeDict):
    sender: str
    savings: str

class DebitMetadata(TypeDict):
    receiver: str
    airtime: str
    savings: str

class Transaction(TypeDict):
    date: str
    credit: bool
    debit: bool
    credit_metadata: Optional[CreditMetadata]
    debit_metadata: Optional[DebitMetadata]
...
def parse_debit_transaction(transaction) -> Transaction:
    res = {"date": transaction['date']}
    if "you just sent" in transaction:
        res["debit"] = True
    
    res_metadata = {}

def parse_credit_transaction(transaction) -> Dict[str, str]:
    pass
...
def unparsed_transactions() -> List[Dict[str, str]] | None:
    ...
    for trxn in batch:
        ...
        trxns.append({"trxn_statement": s.span.get_text(), "date": datetime.strftime(trxn.date, "%Y-%m-%d")})
        counter += 1
    return trxns
